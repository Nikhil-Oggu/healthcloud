...
public class ClaimReviewService {
    ...
    /**
     * A page of reviews in the caller's tenant (header-only), optionally filtered to one claim and/or a status
     * (§Phase 9 — server-side pagination + filtering). Gated by patient (§21 layer 6): a provider sees only reviews
     * for patients they are actively assigned to; broad roles (coordinator/admin/reviewer) see the tenant's reviews
