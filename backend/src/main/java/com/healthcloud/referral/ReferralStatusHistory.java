...
/**
 * An immutable record of one status change on a referral (source-of-truth §31.6): from-state, to-state, actor,
 * organization, reason, timestamp and correlation id. Append-only — never updated or deleted. {@code fromStatus}
 * is null for the creation row (null → REQUESTED). Mirrors {@code PriorAuthorizationStatusHistory}.
 */
@Entity
...
public class ReferralStatusHistory {
    ...
    @Column(name = "referral_id", nullable = false, columnDefinition = "uuid", updatable = false)
    ...
    protected ReferralStatusHistory() {
        // for JPA
    }

    public ReferralStatusHistory(UUID organizationId, UUID referralId, ReferralStatus fromStatus,
                                 ReferralStatus toStatus, UUID actorUserId, String reason, String correlationId) {
