...
- **At a glance** (newest first; the detailed per-phase bullets and the dated log below carry the full record):
  - **Phase 12 COMPLETE ✅** Final validation & portfolio — slice 1 ✅ **full-suite validation run** (measured: **backend 511 tests** / 0 fail via `mvnw clean verify` + Testcontainers; **frontend 183 tests** / typecheck / build; **694 total, all green**) · slice 2 ✅ **README engineering case study** (40-line stub → 15-section TOC'd case study with 2 inline Mermaid diagrams, citing the measured 694; honesty preserved; rendered-verified) · slice 3 ✅ **architecture + ER diagrams** (`docs/architecture/architecture.md` — 4 Mermaid diagrams: deployment/topology, module map, authorization pipeline, event flow; `docs/er-diagram/er-diagram.md` — the 50-table model as 7 domain-grouped `erDiagram` blocks, built from the live schema; all 11 blocks parse-valid; README doc-index links both) · slice 4 ✅ **threat model** (`docs/threat-model/threat-model.md` — STRIDE over the real trust boundaries; a Mermaid DFD + per-category threat tables with every mitigation grounded in real code + honest residual risks; README doc-index links it) · slice 5 ✅ **ADRs** (8 new: 003/005/009/010/011/014/015/016, numbered per the source-of-truth Appendix A read via `pypdf`; then the 6 deferred — 006/007/008/012/013/017 — also written, so **all 17 SoT ADRs + ADR-018 are now recorded**) · slice 6 ✅ **evidence pack + resume bullets** (`docs/evidence/` — 6a a fresh full-suite test capture (measured 511+183=**694** green); 6b **live security-boundary HTTP transcripts** proving cross-tenant secure-404, consent field-masking with a before/after GRANT flip, the relationship gate (same role → 200 vs 404), and 409 invalid-transition; 6c **observability** screenshots (Grafana 18-panel dashboard, Prometheus target UP, Jaeger `adjudicate-claim` trace); 6d **11 app-UI screenshots** of every major screen; 6e **live AWS cloud capture** over HTTPS (current build + branded Cognito login + authenticated ORG_ADMIN session) — the deploy was **rebuilt+redeployed** (current images to ECR) and the Cognito client re-branded to match; 6f **resume bullets** + this doc close-out. Screenshots captured via headless Chrome / playwright-core straight to disk). **Phase 12 COMPLETE — the full 0→12 roadmap is done. ✅**
...
- **Slice 6 built `docs/evidence/`** — the "show me, don't tell me" proof folder — as sub-slices, each committed:
  - **6a — scaffold + fresh tests** (`README.md` index + acceptance-criteria→evidence map; `test-results.md`).
    Re-ran the full suite: **backend 511** (`mvnw clean verify`, Testcontainers) + **frontend 183** + typecheck +
    build = **694 green** (dated 2026-09-25). Noted honestly: one frontend test timed out only when run
    *concurrently* with the backend suite (CPU contention) — passes 18/18 / 183/183 uncontended.
  - **6b — security-boundary transcripts** (`security-proofs.md`) — real captured HTTP request/response proof
    against the running local app: cross-tenant **secure-404**; consent field-masking with a **before/after GRANT
    flip** (DOB `null`+`maskedFields` → real date, `maskedFields:[]`); the **relationship gate** (Dana assigned →
    200 vs Morgan unassigned, same PROVIDER role → 404; list scoping 3/2/0); **409 INVALID_STATE_TRANSITION** on
    illegal claim moves. Strongest, fully-reliable evidence.
  - **6c — observability** (`observability.md` + `screenshots/`) — brought up the `observability` stack, generated
    clean traffic, captured the Grafana "HealthCloud Overview" (18 panels, 0 5xx, adjudications=6), the Prometheus
    target **1/1 UP**, and a Jaeger trace with the custom **`adjudicate-claim`** span nested under the HTTP span.
    Did a **fresh backend restart with Kafka up** first, so metrics show 0 errors (an earlier capture had 7 spurious
    5xx traced to my own malformed debug requests — `NoResourceFoundException` on empty path vars — not a product bug).
  - **6d — app UI** (`ui.md` + `screenshots/ui-*.png`) — **11 full-page screenshots** of every major screen (landing,
    dashboard, patients, patient detail with **DOB "Restricted"** + consent, claim **adjudication breakdown** +
    version history, prior-auth/appeals/requests queues, coverage plans + detail, **audit trail hash-chain**).
  - **6e — live AWS cloud capture** (`aws.md` + `screenshots/aws/`) — `terraform apply` (**7 add, 0 destroy**) →
    live over HTTPS at a CloudFront URL → captured the landing, the **real Cognito hosted login**, and an
    **authenticated ORG_ADMIN session**. First capture exposed two truths: the deployed **frontend image was an
    older build** (manual ECR push cadence) and the **Cognito branding had reverted** (a fresh `apply` recreates the
    client; branding was CLI-applied, not in TF). Fixed by **rebuilding both current images (ARM64) → ECR via crane →
    force-new-deployment**, and **copying the exact hosted-UI CSS + wordmark logo from the local client (2apbhhj…) to
    the deployed client (u6nf6l…)** via `set-ui-customization`, then recapturing — cloud now matches the current build.
  - **6f — resume bullets + close-out** (`resume-bullets.md`) — interview-ready, measured-only bullets + talking
    points; then brought **PROGRESS.md / CLAUDE.md / learning-module.md** current with slice 6.
- **Screenshot-to-disk solved** with **headless Google Chrome** (and **playwright-core** using the installed Chrome
  for the interactive login capture) — PNGs written straight into `docs/evidence/screenshots/`, no manual step.
- **Bug spotted (documented follow-up, not fixed here):** the deployed `/api/v1/auth/config` returns a
  `cognitoLogoutUrl` whose `logout_uri` points to **`http://localhost:5173/`** instead of the CloudFront URL — logout
  on the deploy would bounce to localhost. A deployed-config fix for a later on-demand slice.
- **AWS note:** the deployment was left **up** at the user's request (to explore) — still metering; tear down with
  `terraform destroy` when done. Cognito hosted-UI branding is applied via CLI (not TF) — a known drift item.
- **Phase 12 COMPLETE ✅ — the full Phase 0→12 roadmap is done.** Remaining is optional documented follow-ups + the
  user's own demo video.

### 2026-09-24 — Phase 12 slices 1–5: validation + README + diagrams + threat model + ADRs ✅ (portfolio)
- **Phase 12 (final validation & portfolio) STARTED.** Plan: (1) validation run, (2) README case study,
  (3) architecture + ER diagrams, (4) threat model, (5) remaining ADRs, (6) evidence pack + resume bullets.
- **Slice 1 — validation run (measured, no code change).** Ran the full suite fresh to capture real numbers
  before writing any into docs (rule 2):
  - **Backend** `./mvnw -B clean verify` (Testcontainers → real PostgreSQL per test): **Tests run: 511,
    Failures: 0, Errors: 0, Skipped: 0** → BUILD SUCCESS, jar built + repackaged (2:41). The Kafka `WARN`
    lines are the Kafka-test context shutting down its broker — expected noise, not failures.
  - **Frontend** (Node 24): `tsc --noEmit` clean · Vitest **183 tests / 40 files, 0 failures** · `npm run
    build` succeeds (the >500 kB chunk note is an informational bundling suggestion, not an error).
  - **Total: 694 tests, all green** — the measured facts the README/evidence pack cite.
- **Slice 2 — README as an engineering case study.** Rewrote `README.md` from a 40-line Phase-0 stub into a
  comprehensive, TOC'd case study (15 sections): problem → the layered authorization pipeline (the
  differentiator) → architecture → feature tour → security deep-dive → stack → testing (cites the 694) →
  cloud/observability → run-it-locally → repo layout → honest limitations → docs index. Two **inline Mermaid
  diagrams** (GitHub renders them natively — no image files): the `identity→org→role→relationship→consent→
  purpose→field→allow/deny→audit` flow and a high-level component diagram. Honesty preserved throughout
  (synthetic-data-only, HIPAA-*aligned*-not-certified, AA-*aligned*, on-demand cloud so **no dead live-URL
  link**, alert thresholds = *targets*). Facts verified before writing: 43 Flyway migrations, 30 `com.healthcloud`
  domain packages, Spring Boot 4.1.0/Java 25, React 19.2/TS 6/Vite 8/MUI 9. **Verified rendered** in the browser
  preview (marked + mermaid): both diagrams draw as SVG, all sections/tables present.
- **Editorial decision (deliberate — do NOT re-add):** the recruiter-facing **Documentation index** links only
  portfolio artifacts (source-of-truth, PLAN/PROGRESS, ADRs, runbooks, design system, diagrams-to-come).
  `CLAUDE.md` and `docs/learning/learning-module.md` are **intentionally NOT linked there** — `CLAUDE.md` is an
  internal AI/contributor working-rules file (not a showcase artifact) and the learning module is the user's own
  interview prep. Both files stay in the repo; `CLAUDE.md` is still listed in the factual repo-layout tree (a
  truthful description of the root), just not featured in the curated index. A future README edit must not re-add
  those two links.
- **Slice 3 — architecture + ER diagrams (Mermaid-in-markdown, GitHub-native, no binary images).** Built from
  **ground truth**: the real schema pulled live from the running Postgres (all foreign keys + columns via
  `information_schema`), so nothing is invented (rule 2). Two new docs:
  - **`docs/architecture/architecture.md`** — 4 diagrams + prose: (1) deployment/topology (the on-demand AWS
    shape — CloudFront→ALB→one Fargate task w/ nginx+backend over localhost→RDS; Cognito; Secrets Manager; no
    NAT; no MSK), (2) module map (the ~30 domain packages grouped over the cross-cutting layers +
    `PatientAccessGuard` as the single relationship choke-point), (3) request lifecycle & authorization pipeline
    (tenant→role→relationship→consent→field-mask→allow/deny→audit, + the one-tx write/history/outbox),
    (4) event-driven flow (outbox→relay→Kafka→idempotent consumer→DLT→drain→replay).
  - **`docs/er-diagram/er-diagram.md`** — the **50-table** data model as **7 domain-grouped `erDiagram`
    blocks** (tenancy/identity · care coordination · consent/clinical · coverage plans & config · claims &
    adjudication · advanced claims · security/audit & events), each entity showing key columns (PK/tenant
    key/FKs/business cols) with real names from the live schema; opens with the multi-tenancy `organization_id`
    + FK-with-org pattern and the global-reference-data note (`medical_code`/`role`).
  - **README** Documentation index now links both (replaced the "diagrams … will be linked" placeholder; the
    threat-model line remains as the next slice's placeholder).
  - **Verified:** all **11 Mermaid blocks pass `mermaid.parse`** (the authoritative check — GitHub renders via
    Mermaid); architecture diagrams render visibly in the preview; ERD content confirmed accurate + rendering
    (a plain-text re-render screenshotted cleanly). Note: Mermaid v11 draws ER labels with `foreignObject`,
    which the desktop screenshot pipeline doesn't rasterize (blank in a screenshot) but GitHub renders fine —
    a capture quirk, not a file defect.
- **Slice 4 — threat model (`docs/threat-model/threat-model.md`), STRIDE over the real boundaries.** Every
  mitigation is **grounded in real code** (spot-checked before writing, rule 2 — no overstated protections):
  a Mermaid **data-flow / trust-boundary diagram** (5 boundaries: browser → CloudFront/ALB → app security
  boundary → data/messaging → external IdP/secrets) + **STRIDE tables** (Spoofing/Tampering/Repudiation/Info-
  disclosure/DoS/Elevation), each row *threat → implemented mitigation w/ code pointer → residual*. Real
  mechanisms cited & verified: CSRF handshake (`CookieCsrfTokenRepository.withHttpOnlyFalse`), roles-from-DB
  (`UserContextFilter`), secure-404 tenant scoping + `PatientAccessGuard`, deny-by-default field masking
  (`ConsentPolicyService.decideForActor` → `maskedFields`), HMAC-SHA256 audit hash chain (`AuditHashChain` +
  `AuditSigningKeys`, master secret in config not DB) + verify endpoint, optimistic locking (36 `@Version`
  entities) + `benefit_accumulator` `PESSIMISTIC_WRITE` row lock, idempotent consumer (`existsByEventId` +
  `UNIQUE(event_id)`), engine-owned statuses, time-boxed audited break-glass, `demo,cognito` closing the
  dev-login bypass, Secrets Manager injection (`ecs.tf` `valueFrom`). **Honest §6 residual risks**: no app-level
  rate limiting (DoS mostly out of scope), audit master secret is a config value (KMS/HSM is the prod follow-up),
  MFA optional-not-enforced, ALB origin-verify header follow-up, single-instance relay, incremental audit
  coverage, synthetic-data-only. Plus a §5 defense-in-depth summary + §7 assumptions/trust. README doc index now
  links it. **Verified:** the DFD passes `mermaid.parse` and renders visibly in the preview.
- **Slice 5 — remaining ADRs (8 of the 14 missing, top decisions first).** Read the source-of-truth's **Appendix A**
  to get the canonical ADR titles/numbers — the PDF wouldn't render (no poppler), so extracted the text with
  **`pypdf`** (Python 3.9 system interpreter had it); the user also sent an image of Appendix A that matched the
  extraction exactly, so numbering is confirmed against the SoT. Wrote 8 new ADRs (existing template, Status
  Accepted, each grounded in real code + honest limitations): **003** backend-derived tenant context, **005**
  hybrid authorization (the layered pipeline + `PatientAccessGuard`), **009** transactional outbox + Kafka, **010**
  per-org HMAC audit chains, **011** concurrency strategy (optimistic `@Version` + `benefit_accumulator` row lock),
  **014** immutable consent + claim decisions (supersede + versioned adjudication), **015** on-demand full AWS
  validation (apply→evidence→destroy; no MSK/NAT; ARM64), **016** single-origin browser architecture (CloudFront/
  nginx/Vite proxy + redirect_uri pinning). Updated the ADR README index (now 001–018 with accurate Accepted/Pending
  status). **Deferred (6, all real decisions, just not yet written up):** 006 limited clinical context, 007 bounded
  claims engine, 008 PostgreSQL metadata + private S3 docs, 012 ECS Fargate, 013 Terraform + GitHub Actions, 017
  PostgreSQL-first (no Redis/OpenSearch).
- **Files:** `README.md` (rewritten + doc-index links), `docs/architecture/architecture.md` (new),
  `docs/er-diagram/er-diagram.md` (new), `docs/threat-model/threat-model.md` (new), `docs/adr/ADR-003/005/009/010/
  011/014/015/016-*.md` (8 new), `docs/adr/README.md` (index), `docs/PROGRESS.md`. No app/behavior change.
- **Containers:** stopped the 3 idle observability containers (Grafana/Prometheus/Jaeger — nothing to scrape
  with the backend down); kept `healthcloud-postgres` (also used it to read the live schema for the ERD).
  Relaunch observability on-demand for the evidence slice.
- **Slice 5b — the 6 deferred ADRs written** (completing the full source-of-truth set): **006** limited clinical
  context, **007** bounded synthetic claims engine, **008** PostgreSQL metadata + private S3 documents (honest note:
