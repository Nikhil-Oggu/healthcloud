...
### 2026-09-24 — Phase 12 slices 1–2: full-suite validation + README engineering case study ✅ (portfolio)
...
- **Editorial decision (deliberate — do NOT re-add):** the recruiter-facing **Documentation index** links only
  ...
  those two links.
- **Files:** `README.md` (rewritten), `docs/PROGRESS.md`. No app/behavior change.
- **Containers:** stopped the 3 idle observability containers (Grafana/Prometheus/Jaeger — nothing to scrape
  with the backend down); kept `healthcloud-postgres`. Relaunch observability on-demand for the evidence slice.
- **Next:** slice 3 — architecture + ER diagrams into `docs/architecture/` + `docs/er-diagram/`.
