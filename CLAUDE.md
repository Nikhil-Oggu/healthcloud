...
- **Free-text search (§Phase 9 slices 6–8):** a queue's list endpoint may add a `q` search param alongside the
  ...
  (`lower(cast(e.resourceId as string)) like …`) so a partial-id paste matches. The remaining Phase 9 areas are
  CSV export with masking + WCAG 2.2 AA.
