...
- **CI** (`.github/workflows/ci.yml`, on push + PR to `main`): a **backend** job (Temurin JDK 25 →
  ...
  follow-up); GHA layer cache speeds rebuilds. Pull with `docker pull ghcr.io/nikhil-oggu/healthcloud-backend:latest`
  (the package may need to be made public once in the repo's Packages settings). Keep all three green — don't merge red.
