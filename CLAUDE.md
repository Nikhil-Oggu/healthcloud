...
- **Accessibility (§Phase 9 slice 10, WCAG 2.2 AA-aligned):** the app shell (`AppLayout`) provides the page's
  ...
  **Known limitation (rules 2–3):** axe under jsdom **cannot check color contrast** (no rendering engine — the rule
  ...
  and rely on MUI's AA-designed default theme. So this is AA-*aligned*, not certified — a full page-by-page audit +
...
- **ECS Fargate + ALB (Phase 10 slice 10, `ecs.tf` + `alb.tf`) — the running app.** ONE Fargate task holds **both
  ...
  role** (managed `AmazonECSTaskExecutionRolePolicy` + an inline `secretsmanager:GetSecretValue` on the RDS secret) —
  **never** put a DB password in the task def or env plaintext; the **task role** is the app's own runtime identity
  (empty until S3/Cognito slices need it). Tasks run in the **public subnets with `assign_public_ip = true`** (no NAT
  ...
  so it was left byte-identical to avoid forcing a replacement). The ALB is **HTTP :80 only** for now (HTTPS = ACM cert
  + a domain, deferred to the CloudFront/Cognito slices) — the SESSION cookie is not forced `Secure`, so login works
  over HTTP. For a demoable deploy the backend runs the **`local` profile** (dev-login + synthetic seed) with **Kafka
  off** (`HEALTHCLOUD_OUTBOX_RELAY_ENABLED=false` + `HEALTHCLOUD_KAFKA_CONSUMERS_ENABLED=false` — no MSK); flip to the
  default profile once Cognito lands. **This slice starts the hourly meter** (~$0.08–0.10/hr: ALB + Fargate + public
