...
- **Backend packages** under `com.healthcloud`: `organization` (Organization, Facility, FacilityMembership),
  ...
  **`PatientAccessGuard` consults live grants** (see the Authorization-layering convention): a provider-gated caller
  ...
  emergencies); audited at invocation, not per subsequent read; grants expire on their own (no early revocation yet).
  **UI (slice 5):** a provider-facing break-glass panel on a patient's denied page + a `/break-glass` grants list —
  see `src/breakglass/` under Frontend below. **Deferred:** admin/auditor oversight of all grants folds into access reviews),
