...
- **Free-text search (§Phase 9 slice 6):** a queue's list endpoint may add a `q` search param alongside the
  ...
  strings). **All six numbered work queues now have it** (claims — slice 6; prior-auth, referrals, appeals,
  claim-reviews, reprocessing — slice 7). The two remaining queues (audit, dead-letters) have no business number,
  so their search targets differ (audit → correlationId/resourceId; dead-letters → eventId) and are a later
  slice, along with CSV export with masking + WCAG 2.2 AA.
