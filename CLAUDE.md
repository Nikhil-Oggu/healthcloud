...
- **Container image (Phase 10 slice 1):** `docker build -t healthcloud-backend:local ./backend` builds the
  production image; `docker compose --profile full up -d postgres backend` runs it against the Postgres
  container (default Spring profile → Flyway migrates, no seed/dev-login; add `SPRING_PROFILES_ACTIVE: local`
