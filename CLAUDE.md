...
- **CloudFront HTTPS (Phase 10 slice 15, `cloudfront.tf`) — the public HTTPS entry.** A distribution in front of the
  ...
  tier), ~3–15 min to create/update. Follow-ups: logout is local-only (no RP-initiated Cognito logout); the ALB stays
  directly HTTP-reachable (lock it to CloudFront origins to harden).
