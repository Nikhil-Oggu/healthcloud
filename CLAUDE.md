...
- **Terraform (IaC) conventions.** Lives in `infrastructure/terraform/`; pin `required_version` + the AWS provider
  ...
  local for resource names. **Local state backend for now** (state files are git-ignored — never commit `*.tfstate`
  or `.terraform/`); the S3 + DynamoDB remote backend is bootstrapped in a later (AWS-cost) slice. All deployed
  data stays **synthetic only**.
