...
- **Run app with real Cognito login (Phase 10 slice 12):** run the backend with `SPRING_PROFILES_ACTIVE=local,cognito`
  ...
  --username <email> --password '…' --permanent`). **ALL 14 seeded users are now Cognito-enabled (CONFIRMED) in the
  pool (2026-09-21):** every role — `patient@`/`provider@`/`provider2@`/`coordinator@`/`reviewer@`/`admin@`/`auditor@`
  — for **both** `northcare.example.org` and `greenvalley.example.org`. (Originally only the 4 providers + 2 admins;
  the remaining 8 — patients, coordinators, reviewers, auditors × both orgs — were enabled the same way so a stranger
  can log in as any role and walk the full cross-role workflow on the deployed demo.) Add any future user the same way
  (`admin-create-user --message-action SUPPRESS` then `admin-set-user-password --permanent`). NB: setting
...
to the AWS deployment, Alertmanager routing, RDS PITR/snapshot DR — all on-demand + approval-gated. **Phase 12 (final validation & portfolio) COMPLETE ✅ — the full Phase 0→12 roadmap is done** — slices 1–5 done: full-suite validation (measured — backend **511** tests via `mvnw clean verify` + Testcontainers, frontend **183** tests + typecheck + build = **694 green**), the **README** rewritten as a 15-section engineering case study (with 2 inline Mermaid diagrams), **architecture + ER diagrams** (`docs/architecture/architecture.md` — 4 Mermaid diagrams; `docs/er-diagram/er-diagram.md` — the 50-table model as 7 domain-grouped `erDiagram` blocks, built from the live schema), the **STRIDE threat model** (`docs/threat-model/threat-model.md` — a DFD + per-category threat tables, every mitigation grounded in real code), and the **ADR set completed** (all 17 source-of-truth ADRs 001–017 + ADR-018 now written in `docs/adr/`; the SoT Appendix A titles were read from the PDF via `pypdf`). **Slice 6 done: the evidence pack** (`docs/evidence/`) — 6a fresh test capture (`test-results.md`, measured 694); 6b **live security-boundary HTTP transcripts** (`security-proofs.md` — cross-tenant secure-404, consent field-masking before/after a GRANT flip, the relationship gate same-role 200-vs-404, 409 invalid-transition); 6c **observability** screenshots (`observability.md` — Grafana 18-panel, Prometheus target UP, Jaeger `adjudicate-claim` trace); 6d **11 app-UI screenshots** (`ui.md`); 6e **live AWS cloud capture** (`aws.md` — current build over HTTPS + branded Cognito login + authenticated ORG_ADMIN); 6f **resume bullets** (`resume-bullets.md`). **Screenshots were captured with headless Chrome / playwright-core straight to disk** (the reliable way to get PNGs into the repo — the browser pane can't save to disk). For 6e the **deploy was rebuilt+redeployed** (the live deploy had lagged behind all the post-Phase-11 UI work because ECR images are pushed manually, not from CI) and the **Cognito hosted-UI branding was copied from the local client (2apbhhj…) to the deployed TF client (u6nf6l…)** via `set-ui-customization` (branding is per-client and not in Terraform → a fresh `apply` reverts it; documented drift). **Bug found (documented follow-up):** the deployed `/api/v1/auth/config` `cognitoLogoutUrl` has `logout_uri=http://localhost:5173/` instead of the CloudFront URL — deployed logout bounces to localhost. Remaining beyond the roadmap: optional documented follow-ups + the user's own demo video. See docs/PROGRESS.md for status)
...
- **Auth:** session-based; `SESSION` cookie (HttpOnly), Spring Session JDBC (tables in `spring_session`);
  ...
  **Sign-in capability probe (UI-polish pass):** a **public** `GET /api/v1/auth/config` (`AuthConfigController`,
  permit-all in `SecurityConfig`) returns `{ cognitoEnabled, cognitoLogoutUrl }` — `cognitoEnabled` true only when a
  `ClientRegistrationRepository` with a `cognito` registration exists. It exposes only the boolean + a public logout
  URL (no secrets/ids). The login page reads it pre-auth and **disables the "Sign in with Cognito" button with an
  explanation when Cognito isn't configured** (e.g. local dev without the `cognito` profile), instead of letting a
  click hit `/oauth2/authorization/cognito` and its 500. The deployed `demo,cognito` app returns `cognitoEnabled:
  true`, so the button is live there.
  **RP-initiated logout (2026-09-23, commit `7cad836`):** the same `/api/v1/auth/config` also returns
  **`cognitoLogoutUrl`** — Cognito's **non-standard** hosted-UI logout URL
  (`https://<hosted-ui-domain>/logout?client_id=..&logout_uri=..`, NOT the OIDC `end_session_endpoint`, which
  Cognito doesn't advertise — so Spring's `OidcClientInitiatedLogoutSuccessHandler` can't be used; the URL is built
  by `AuthConfigController` from `healthcloud.cognito.hosted-ui-domain` + `logout-redirect-uri` in
  `application-cognito.yml` (env-overridable) + the registered client id), or `null` when Cognito isn't configured.
  The SPA's `AppLayout.handleLogout` calls `api.logout()` (clears the Spring `SESSION` cookie) then
  **full-page-redirects to `cognitoLogoutUrl`** so Cognito's own SSO cookie is cleared too — otherwise the next
  "Sign in" silently re-authenticates the same user (you'd need an Incognito window to switch users). It falls back
  to a client-side `/login` navigation when the URL is absent (dev without Cognito). The `logout-redirect-uri` must
  be a **registered "Sign out URL"** on the app client (localhost:5173 locally — already registered; the CloudFront
  URL on deploy — already in `cognito.tf`), so **no AWS change was needed**. This closed the Phase-10 "logout is
...
- **CloudFront HTTPS (Phase 10 slice 15, `cloudfront.tf`) — the public HTTPS entry.** A distribution in front of the
  ...
  tier), ~3–15 min to create/update. (Follow-up **DONE** 2026-09-23, commit `7cad836`: **RP-initiated Cognito
  logout** — logout now clears the Cognito hosted-UI cookie too; see the RP-initiated-logout note in the Auth section.)
- **Auth-hardening slice (Phase 10, post-slice-15) — the deployed auth boundary is real.** Three fixes from the
  ...
  (the prefix list still admits any account's CloudFront), a custom domain, enforce MFA. (**RP-initiated Cognito
  logout is now DONE** — 2026-09-23, commit `7cad836`.)
